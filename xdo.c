...
int xdo_get_window_role(const xdo_t *xdo, Window window,
                        ...
                        int *role_type) {
  Atom type = XInternAtom(xdo->xdpy, "WM_WINDOW_ROLE", False);
  int size;
  long nitems;

  *role_ret = xdo_get_window_property_by_atom(xdo, window, type, &nitems,
                             &type, &size);
...
int xdo_get_window_class(const xdo_t *xdo, Window window, unsigned char **class_ret) {
  ...
  if (ret) {
    ...
    *class_ret = (unsigned char*) classhint.res_name;
  } else {
    *class_ret = NULL;
  }
